...
task("test-with-fork", "Test certificate minting")
  .setAction(async (args, hre) => {
    ...
    try {
      ...
      const tx3 = await instance.connect(minter).mint(minter.address, {
    ...
    try {
      ...
      const tx5 = await instance.connect(minter).mint(minter.address, {
        value: ethers.parseEther("2")
      });
      await tx5.wait();
    }
    ...
    try {
      ...
      const tx6 = await instance.connect(minter).mint(deployer.address, {
    ...
    try {
      ...
      const tx7 = await instance.connect(minter).mint(minter.address, {
        value: ethers.parseEther("2.1")
      });
      await tx7.wait();
